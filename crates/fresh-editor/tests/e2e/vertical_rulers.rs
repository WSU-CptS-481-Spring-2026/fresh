...
/// Screen width of the left gutter for the active buffer (line digits grow with line count).
fn content_gutter_width(harness: &EditorTestHarness) -> u16 {
    harness
        .editor()
        .active_state()
        .margins
        .left_total_width() as u16
}
...
fn test_rulers_render_at_correct_columns() {
    ...
    assert!(
        has_ruler_bg(&harness, content_gutter_width(&harness) + 10, row),
        "Ruler bg should appear at column 10"
    );

    // Ruler at column 20 should have ruler bg
    assert!(
        has_ruler_bg(&harness, content_gutter_width(&harness) + 20, row),
        "Ruler bg should appear at column 20"
    );

    // Column 15 should NOT have ruler bg
    assert!(
        !has_ruler_bg(&harness, content_gutter_width(&harness) + 15, row),
        "Column 15 should not have ruler bg"
    );

    // Rulers should preserve text content (not overwrite with │)
    let cell_10 = harness.get_cell(content_gutter_width(&harness) + 10, row);
    assert_eq!(
        cell_10.as_deref(),
        Some("X"),
...
fn test_rulers_span_full_height() {
    ...
    let ruler_x = content_gutter_width(&harness) + 10;
...
fn test_rulers_horizontal_scroll() {
    ...
    let ruler_screen_x = content_gutter_width(&harness) + 5;
    assert!(
...
fn test_no_rulers_by_default() {
    ...
    for col_offset in [10u16, 20, 30, 40] {
        assert!(
            !has_ruler_bg(&harness, content_gutter_width(&harness) + col_offset, row),
...
fn test_per_buffer_ruler_independence() {
    ...
    let (content_first_row, _) = harness.content_area_rows();
    let row = content_first_row as u16;
    let ruler_x = content_gutter_width(&harness) + 15;
...
fn test_add_ruler_command() {
    ...
    let (content_first_row, _) = harness.content_area_rows();
    let row = content_first_row as u16;
    let ruler_x = content_gutter_width(&harness) + 25;
...
fn test_remove_ruler_command() {
    ...
    let (content_first_row, _) = harness.content_area_rows();
    let row = content_first_row as u16;
    let ruler_x_10 = content_gutter_width(&harness) + 10;
    let ruler_x_20 = content_gutter_width(&harness) + 20;
...
fn test_add_ruler_invalid_input() {
    ...
    for col in [10u16, 20, 30] {
        assert!(
            !has_ruler_bg(&harness, content_gutter_width(&harness) + col, row),
...
fn test_add_then_remove_ruler_bad_then_good_input() {
    ...
    let (content_first_row, _) = harness.content_area_rows();
    let row = content_first_row as u16;
    let ruler_x = content_gutter_width(&harness) + 80;
