...
impl FileExplorerRenderer {
    ...
    pub fn render_content(
        ...
        // Available width for content (subtract cursor indicator column)
        ...
        if is_focused {
            if let Some(selected) = selected_index {
                if selected >= scroll_offset && selected < scroll_offset + viewport_height {
                    // Position at the left edge of the selected row
                    let cursor_x = area.x;
                    let cursor_y = area.y + (selected - scroll_offset) as u16;
